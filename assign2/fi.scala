...
object Images {
  ...
  // !!FILL ME IN
  def lift1[A, B](f: A => B)(i: Img[A]): Img[B] = (p: Point) => f(i(p))
  ...
  // of type C.
  // !!FILL ME IN
  def lift2[A, B, C](f: (A, B) => C)(a: Img[A])(b: Img[B]): Img[C] = (p: Point) => f(a(p), b(p))
  ...
  // !!FILL ME IN
  def lift3[A, B, C, D](f: (A, B, C) => D)(a: Img[A])(b: Img[B])(c: Img[C]): Img[D] = (p: Point) => f(a(p), b(p), c(p))
  ...
  // colour images.
  // !!FILL ME IN
  val lerpImgClr: Img[Value] => Img[Colour] => Img[Colour] => Img[Colour] = (a: Img[Value]) => (b: Img[Colour]) => (b2: Img[Colour]) => lift3(lerpClr)(a)(b)(b2)
  ...
  // !!FILL ME IN
  val overlayImgClr: Img[Colour] => Img[Colour] => Img[Colour] = (a: Img[Colour]) => (b: Img[Colour]) => lift2(overlayClr)(a)(b)
  ...
  // ImgMask argument.
  // !!FILL ME IN
  val selectImgClr = {(a: Img[Boolean]) => (b: Img[Colour]) => (b2: Img[Colour]) => lift3((bool: Boolean, c: Colour, c2: Colour) => if(bool) c else c2)(a)(b)(b2)}
  ...
  // !!FILL ME IN
  val msk2clr: Img[Boolean] => Img[Colour] = (a: Img[Boolean]) => lift1((bool: Boolean) =>if(bool) black else white)(a)
  ...
  // !!FILL ME IN
  val gray2clr: Img[Value] => Img[Colour] = (a: Img[Value]) => lift1((b: Value) => Colour(b,b,b,1))(a)
}
...
object SpatialTransforms {
  ...
  // y coordinate.
  // !!FILL ME IN
  def translatePt(v: Vector): TransformPt = (p: Point) => Point(v.x+p.x, v.y+p.y)
  ...
  // !!FILL ME IN
  def swirlPt(theta: Double): TransformPt = (p: Point) => rotatePt(sqrt((p.x*p.x)+(p.y*p.y))* (2*PI/theta))(p)

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TransformPt.
  // !!FILL ME IN
  def polarXf(t: TransformPlr): TransformPt = (p: Point) => plr2pt(t(pt2plr(p))) 
  ...
  // !!FILL ME IN
  def translate[A](v: Vector): Filter[A] = (a: Img[A]) => (p: Point) => a(translatePt(Vector(-1*v.x, -1*v.y))(p))
  ...
  // !!FILL ME IN
  def scale[A](v: Vector): Filter[A] = (a: Img[A]) => (p: Point) => a(scalePt(v.inverse)(p))
  ...
  // !!FILL ME IN
  def rotate[A](d: Double): Filter[A] = (a: Img[A]) => (p: Point) => a(rotatePt(-1*d)(p))
  ...
  // !!FILL ME IN
  def swirl[A](d: Double): Filter[A] = (a: Img[A]) => (p: Point) => a(swirlPt(-1*d)(p))
}
...
object Masking {
  ...
  // !!FILL ME IN
  val notM: Img[Boolean] => Img[Boolean] = (a: Img[Boolean]) => lift1((bool: Boolean) => if(bool) false else true)(a)
  ...
  // !!FILL ME IN
  val intersectM: Img[Boolean] => Img[Boolean] => Img[Boolean] = (a: Img[Boolean]) => (b: Img[Boolean]) => lift2((bool: Boolean, bool2: Boolean) => if(bool && bool2) true else false)(a)(b)
  ...
  // !!FILL ME IN
  val unionM: Img[Boolean] => Img[Boolean] => Img[Boolean] = (a: Img[Boolean]) => (b: Img[Boolean]) => lift2((bool: Boolean, bool2: Boolean) => if(bool || bool2) true else false)(a)(b)
  ...
  // !!FILL ME IN
  val xorM: Img[Boolean] => Img[Boolean] => Img[Boolean] = (a: Img[Boolean]) => (b: Img[Boolean]) => lift2((bool: Boolean, bool2: Boolean) => if(bool ^ bool2) true else false)(a)(b)
  ...
  // !!FILL ME IN
  val diffM: Img[Boolean] => Img[Boolean] => Img[Boolean] = (x: Img[Boolean]) => (y: Img[Boolean]) => intersectM(x)(notM(y))
}
